...
const OPENAI_API_KEY = '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'; // Use environment variable for API key
...
// Consolidated messageCreate event listener
client.on('messageCreate', async message => {
    ...
    // Check if the message includes any of the triggers for GPT response
    if (triggers.some(trigger => lowerCaseMessage.includes(trigger))) {
        const response = await fetchGPTResponse(lowerCaseMessage);
        message.reply(response);
        return; // Return after sending GPT response to prevent further processing
    }
    ...
    if (lowerCaseMessage === 'compliment me') {
        const compliment = compliments[Math.floor(Math.random() * compliments.length)];
        message.reply(compliment);
    }

    // ... (Continue with the rest of your functionalities)
});
